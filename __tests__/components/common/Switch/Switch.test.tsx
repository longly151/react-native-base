...
test('should render correctly', () => {
  const { toJSON, getByTestId } = render(<Switch />);
...
test('should have working props', () => {
  const { getByTestId } = render(
    <Switch className="flex-1 flex-row self-center justify-between items-center rounded border mx-1 my-1 p-1 border-sky-700 bg-sky-500" />,
  );

  expect(getByTestId('Switch')).toHaveStyle({
    flexGrow: 1,
    flexShrink: 1,
    flexDirection: 'row',
    alignSelf: 'center',
